...
# Added varying noise
def make_density(W, hard_zero_mask, G_density, L_density, rng, base_noise):
    G_density = float(np.clip(G_density, 0.0, 1.0))
    L_density = float(np.clip(L_density, 0.0, 1.0))
    ...
    if L_density > 0.0:
        noise_sigma = base_noise + 18.0 * L_density
        z = gaussian_filter(rng.normal(size=W.shape), noise_sigma)
        z = (z - z.mean()) / (z.std() + 1e-8)
        z_std = 0.25 + 0.55 * L_density
        multiplier = np.exp(L_density * z_std * z)
...
# Add anisotropy and hotspots
# def make_density(W, hard_zero_mask, G_density, L_density, rng, anisotropy=None, hotspots=None):
#     G_density = float(np.clip(G_density, 0.0, 1.0))
#     L_density = float(np.clip(L_density, 0.0, 1.0))
...
#         # General noise
#         noise_sigma = 30.0 + 18.0 * L_density
#         z = gaussian_filter(rng.normal(size=W.shape), noise_sigma)
#         z = (z - z.mean()) / (z.std() + 1e-8)
...
#         if anisotropy:
#             H, W_dim = shape
#             y_grad, x_grad = np.ogrid[:H, :W_dim]
#             # Generate random directional for gradient angle
#             angle = rng.uniform(0, 2 * np.pi) 
#             gradient = np.sin(angle) * (y_grad / H) + np.cos(angle) * (x_grad / W_dim)
#             z += 0.8 * gradient
#             z = (z - z.mean()) / (z.std() + 1e-8)
