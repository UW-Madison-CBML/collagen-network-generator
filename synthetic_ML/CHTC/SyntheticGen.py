...
def generate_synthetic_shg(
    ...
    D = vecfield.make_density(W, hard_zero_mask, G_density, L_density, rng, base_noise)
...
# sinusoidal_fiber_offset(pts: np.ndarray, rng: np.random.Generator,
...
def generate_synthetic_shg_3d(
    ...
    D = vecfield.make_density(W, hard_zero_mask, G_density, L_density, rng, anisotropy=anisotropy, hotspots=hotspots)
    Qx_g, Qy_g = vecfield.make_global_orientation(X.shape, G_align, G_curve, rng)
    Qx_w, Qy_w, influence = vecfield.make_well_orientation(X, Y, Qx_g, Qy_g, wells)
    Qx, Qy = vecfield.relax(
        Qx_g.copy(), Qy_g.copy(), Qx_g, Qy_g, Qx_w, Qy_w, influence, D, G_align, L_align,
    ...
    # Spline stage
    H, Wpx = Qx.shape
    seeds = splinesamp.sample_seeds_from_density(D, int(spline_num*depth), L_density, rng)
...
#     )
    ...
    splines = np.array(splines, dtype=object)
    layers = np.array_split(splines, n_layers, axis=0)
    ...
    total_image = np.zeros_like(imgs[0])
    for i, img in enumerate(imgs):
        d_val = i*depth/n_layers
        defocus = abs(d_val - focal_depth)
        sigma = float(defocus * depth_blur_scale)

        total_image += gaussian_filter(img, sigma=sigma)
    



    if not minimal:
        plt.imsave(f"{save_prefix}_raster.png", total_image, cmap="gray")
    ...
    if show_plots:
        ...
        # Final rasterized image
        ax[2].imshow(total_image, cmap="gray")
        ax[2].set_title("Rasterized Image")
        ax[2].axis("off")
